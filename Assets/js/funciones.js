let tblUsuarios, tblEst, tblCategoria, tblAutor, tblEditorial, tblLibros, tblPrestar;
...
function registrarLibro(e) {
    ...
    || categoria.value == '' || cantidad.value == '' || num_pagina.value == '') {
        alertas('Todo los campos son requeridos', 'warning');
    } else {
...
function btnEditarLibro(id) {
    ...
    http.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            ...
            // Cargar opciones para select2
            cargarOpcionesSelect2('autor', res.id_autor);
            cargarOpcionesSelect2('editorial', res.id_editorial);
            cargarOpcionesSelect2('categoria', res.id_categoria);
...
function cargarOpcionesSelect2(selector, valorSeleccionado) {
    // Realizar una solicitud AJAX para obtener las opciones
    const urlOpciones = base_url + 'Libros/obtenerOpciones/' + selector;
    $.ajax({
        url: urlOpciones,
        dataType: 'json',
        success: function (data) {
            // Llenar el select2 con las opciones
        ...
        error: function (xhr, status, error) {
            console.error('Error al cargar opciones para ' + selector + ':', status, error);
        }
    });
}
